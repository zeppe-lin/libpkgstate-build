...
printf '%s\n' "$public" | grep -F 'libpkgbuild >=2.0.0' >/dev/null || { echo 'pkgconfig-metadata: missing public libpkgbuild >=2.0.0' >&2; exit 1; }
printf '%s\n' "$public" | grep -F 'libpkgimage >=0.4.0' >/dev/null || { echo 'pkgconfig-metadata: missing public libpkgimage >=0.4.0' >&2; exit 1; }
