...
for f in README.md HISTORY.md CONTRIBUTING.md MAINTAINING.md Doxyfile docs/architecture.md docs/integration.md docs/testing.md docs/abi.md docs/code-style.md docs/meson.build docs/history/libpkgstate-2.5.1-origin.sha256 .clang-format .editorconfig man/libpkgstate-build.3.scdoc; do [ -s "$root/$f" ] || fail "missing $f"; done
for s in "$root"/ci/*.sh "$root"/tests/*.sh; do sh -n "$s" || fail "invalid shell: ${s#$root/}"; done
for file in abi/libpkgstate-build.exports include/libpkgstate-build/export.h tools/generate-elf-export-script.sh ci/qualify-installed.sh ci/installed-build-consumer.cpp; do [ -s "$root/$file" ] || fail "missing $file"; done
...
[ ! -e "$root/man/pkgstate_build_adapter.3.scdoc" ] || fail "obsolete in-tree manual retained: pkgstate_build_adapter.3.scdoc"
